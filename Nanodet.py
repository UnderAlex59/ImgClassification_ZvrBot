...
class NanoDet:
    ...
    def __init__(
        ...
        for i in range(len(self.strides)):
            featmap_size = (
                int(self.image_shape[0] / self.strides[i]),
                int(self.image_shape[1] / self.strides[i]),
            )
    ...
    def post_process(self, preds):
        ...
        for stride, cls_score, bbox_pred, anchors in zip(
            self.strides, cls_scores, bbox_preds, self.anchors_mlvl
        ):
            if cls_score.ndim == 3:
        ...
        if len(indices) > 0:
            ...
            return np.concatenate(
                [det_bboxes, det_conf.reshape(-1, 1), det_classid.reshape(-1, 1)],
                axis=1,
            )
        else:
            return np.array([])
