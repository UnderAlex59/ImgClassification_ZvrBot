...
def main():
    ...
    classifier = Classifier()#Создаем экземпляр классификатора, с которым мбудем дальше работать

    bot = telebot.TeleBot('7330326462:AAHquF2epd3qhhV6psyF2n2el4ez6P68HxA')#Подключаемся в боту по специальному ключу
    ...
    def start(message):
        ...
        bot.send_message(message.from_user.id,
                         format('Привет, я бот для классификации изображений\n↓↓↓Выберите желаемое действие↓↓↓'),
                         reply_markup=markup)
    ...
    def text_handler(message):
        ...
        if (message.text == "Приступить к классификации"):
            ...
            markup.add(btn1, btn2,back)
            bot.send_message(message.from_user.id, "Чем хотите классифицировать ?", reply_markup=markup)
    ...
    def photo_handler(message : telebot.types.Message):
        ...
        s3 = boto3.client('s3', endpoint_url='https://s3.storage.selcloud.ru', region_name='ru-1a',
